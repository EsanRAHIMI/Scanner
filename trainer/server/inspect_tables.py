...
def inspect():
    ...
    if api_key1 and base_id1 and table1:
        url1 = f"https://api.airtable.com/v0/{base_id1}/{quote(table1, safe='')}?pageSize=1"
        try:
            data = airtable_read(url1, api_key1)
            records = data.get("records", [])
            print(f"  Sample record fields: {list(records[0].get('fields').keys()) if records else 'No records'}")
        except Exception as e:
            print(f"  Error reading Table 1: {e}")

    print(f"\nTable 2: {table2}")
    if api_key2 and base_id2 and table2:
        url2 = f"https://api.airtable.com/v0/{base_id2}/{quote(table2, safe='')}?pageSize=1"
        try:
