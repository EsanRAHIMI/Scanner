...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "not the main Products catalog. ALWAYS call get_visible_import_context for questions "
    "about the import list, row counts, Matched/Unmatched/Empty status, or why rows "
    "failed to match. Do NOT use get_selected_products or get_visible_products_context "
    "on this page.\n\n"
    "Import match rules (returned by get_visible_import_context → match_analysis):\n"
    "- Matched: value in any selected Excel column equals a product value in the chosen "
    "Products column (trim + lowercase).\n"
    "- Unmatched: Excel row HAS a value in the match column(s) but NO product has that "
    "value in the Products column — usually new products, wrong column, typo, or formatting.\n"
    "- Empty: all selected Excel match columns are blank.\n"
    "When the user asks why rows are Unmatched, answer from match_analysis (counts, "
    "why_unmatched, likely_causes, unmatched_samples). Never ask them to provide details "
    "you already have in context or tool results.\n\n"
