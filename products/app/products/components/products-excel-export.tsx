...
export function ProductsExcelExport({
  ...
  const [selectedColumns, setSelectedColumns] = React.useState<Set<string>>(() => new Set(allColumns));
  ...
  const columnsKey = allColumns.join('\u0000');
  ...
  const handleExport = React.useCallback(async () => {
    setError(null);
    const exportColumns = allColumns.filter((column) => selectedColumns.has(column));
    setExporting(true);
    try {
  ...
  }, [allColumns, records, selectedColumns]);
